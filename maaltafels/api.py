...
from maaltafels import __version__, authenticated, db, server
...
server.json_encoder = CustomJSONEncoder  # type: ignore[attr-defined]
  ...
  def get(self):
    return list(
      db.results.aggregate(
        [
          {"$match": {"_user": request.authorization.username}},
          {
            "$project": {
              "year": {"$year": "$_ts"},
              "month": {"$month": "$_ts"},
          ...
          {
            "$group": {
              "_id": {"$dateFromParts": {"year": "$year", "month": "$month", "day": "$dayOfMonth"}},
              "questions": {"$sum": 1},
...
class Sessions(Resource):
  ...
  def get(self, id=None):
    return list(
      db.sessions.find(
        ...
        {"start": 1, "end": 1, "tables": 1, "duration": 1, "_id": 0},
      )
      .sort([("start", -1)])
      .limit(10)
    )
  ...
  def put(self, id):
    ...
    db.sessions.update_one(
      {"_id": ObjectId(id), "_user": request.authorization.username}, {"$set": update}
    )


api.add_resource(Sessions, "/api/sessions", "/api/sessions/<string:id>")
...
api.add_resource(Coverage, "/api/coverage")
