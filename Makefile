...
test-cov: env-dev ## Run tests with coverage
	uv run pytest --cov=maaltafels --cov-report=term-missing $(TEST)
...
format: env-dev ## Format code and fix linting issues
	uv run ruff format maaltafels tests
	uv run ruff check --fix maaltafels tests

lint: env-dev ## Check code for linting issues
	uv run ruff check maaltafels tests

typecheck: env-dev ## Run type checking
	uv run mypy maaltafels
